...
const complexityOptions = [
  { value: 'basic', label: 'Basic CRUD', desc: 'Simple data operations', icon: <FiDatabase className="w-5 h-5" /> },
  { value: 'saas', label: 'Multi-tenant SaaS', desc: 'Isolated tenants & billing', icon: <FiLayers className="w-5 h-5" /> },
  { value: 'complex', label: 'Complex Workflows', desc: 'Multi-step processes', icon: <FiServer className="w-5 h-5" /> },
]
          ...
          <div className="grid grid-cols-3 gap-3">
            {complexityOptions.map((opt) => (
              <button
                ...
                disabled={loading}
                className={`p-4 rounded-xl border text-left transition-all duration-200 ${complexity === opt.value ? 'border-primary bg-primary/10 glow-primary' : 'border-border bg-card hover:border-primary/30'}`}
              >
          ...
          <div className="text-center space-y-3">
            <div className="flex justify-center gap-1">
              {stages.map((_, i) => (
                <div
                  key={i}
                  className={`h-1.5 rounded-full transition-all duration-500 ${i === stageIdx ? 'w-8 bg-primary' : 'w-2 bg-muted'}`}
                />
              ))}
            </div>
